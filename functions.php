...
class Functions
{
    ...
    public function getComments($mysqli) {
		...
		if (!($stmt = $mysqli->prepare("CALL getComments()"))) {
		    $data = array(
				"success"	=> false,
		...
		do {
		    ...
		    if (!$stmt->bind_result($cID,$cS,$cB,$cStar,$cSeen,$cDateTime)) {
		        $data = array(
					"success"	=> false,
	...
	# ==== insert user in DB ==== #
	# =========================== #
    public function updateSeen($mysqli) {
	    $resultArray = array();

		if (!($stmt = $mysqli->prepare("CALL updateSeen(?)"))) {
		    $data = array(
				"success"	=> false,
				"message"	=> "Prepare failed: (" . $mysqli->errno . ") " . $mysqli->error
			);
    ...
    public function insertComment($mysqli) {
		...
		if (!($stmt = $mysqli->prepare("CALL insertComment(?,?,?,?)"))) {
		    $data = array(
				"success"	=> false,
				"message"	=> "Prepare failed: (" . $mysqli->errno . ") " . $mysqli->error
			);
		...
		$commentSeen = $_POST['commentSeen'];

		if (!$stmt->bind_param("ssss", $commentS, $commentB, $commentStar, $commentSeen)) {
		    $data = array(
				"success"	=> false,
				"message"	=> "Binding parameters failed: (" . $stmt->errno . ") " . $stmt->error
			);
